...
import { usePointer } from './interaction/pointer';
import { LiquidBody, type Ball } from './components/LiquidBody';
import { LiquidGlass } from './materials/LiquidGlass';
import { expDamp } from './sim/damp';
...
//
// Next SSR note: this touches window + WebGL, so the page loads it via next/dynamic
// { ssr:false } (see ThreadSurface). p5 is NOT used — the lab hero is R3F.
...
function Orb() {
  ...
  const balls = (t: number, dt: number, out: Ball[]) => {
    ...
    if (coreMat.current)
      coreMat.current.emissiveIntensity = Math.max(0, 0.07 + 0.08 * Math.sin(t * 1.4)) + a * 3.8;
    return out;
  };

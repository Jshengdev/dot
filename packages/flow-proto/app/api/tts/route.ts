...
// reaches the client. POST { text } → audio/mpeg. Verified live: 200, 24kHz mp3.
...
export async function POST(req: NextRequest) {
  ...
  if (!upstream.ok) {
    ...
    return Response.json({ error: `grok tts ${upstream.status}`, detail: detail.slice(0, 300) }, { status: 502 });
  }
